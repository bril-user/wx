...
//获取应用实例
const app = getApp()
const devicesId = "643954257" // 填写在OneNet上获得的devicesId 形式就是一串数字 
const api_key = "oBpMGlpgwi0xRE7E4zrGWYR8EqQ=" // 填写在OneNet上的 api-key 
...
Page({
  ...
  getDatapoints: function () {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=HeartBeat&limit=20`,
        /**
         * 添加HTTP报文的请求头, 
        ...
        success: (res) => {
          ...
          resolve({
            HeartBeat: response.data.datastreams[0].datapoints.reverse(),
  ...
  update: function (datapoints) {
    const BodyData = this.convert(datapoints);

    this.lineChart_hum.updateData({
      categories: BodyData.categories,
      series: [{
        name: 'HeartBeat',
        data:BodyData.HeartBeat,
  ...
  convert: function (datapoints) {
    ...
    var length = datapoints.HeartBeat.length
    for (var i = 0; i < length; i++) {
      categories.push(datapoints.HeartBeat[i].at.slice(5, 19));
      HeartBeat.push(datapoints.HeartBeat[i].value);
  ...
  firstDraw: function (datapoints) {
    ...
    var BodyData = this.convert(datapoints);
